        ...
        return (
            <div className="flex items-start gap-3 py-2 border-b border-dashed">
                ...
                <div>
                    ...
                    <div className="text-sm text-muted-foreground prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: displayValue }} />
                </div>
            </div>
        );
     }
    ...
    <Card className="overflow-hidden shadow-xl">
      <CardHeader className="bg-muted p-6">
        <div className="flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="flex-1">
            <Badge variant={person.source === 'fbi' ? 'destructive' : 'default'} className="mb-2">
              {person.source.toUpperCase()} ID: {person.rawId}
            </Badge>
            ...
            <CardDescription className="text-lg text-muted-foreground">
              {fbiData?.subjects?.join(' / ') || interpolData?.arrest_warrants?.map(aw => aw.charge).join(' / ') || 'Crime details not specified.'}
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
        <div className="space-y-2">
          ...
          {fbiData?.age_range && <DetailItem icon={CalendarDays} label="Age Range" value={fbiData.age_range} />}
          ...
          <DetailItem icon={Globe} label="Nationality" value={person.nationality} />
        ...
        <div className="md:col-span-2 space-y-2">
          <h3 className="text-xl font-semibold text-primary mb-3 font-headline">Case Information</h3>
          ...
          {person.details && <DetailItem icon={FileText} label="Details" value={person.details} />}
          {person.remarks && <DetailItem icon={FileText} label="Remarks" value={person.remarks} />}
          {fbiData?.additional_information && <DetailItem icon={FileText} label="Additional Information" value={fbiData.additional_information} />}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {person.images.slice(1).map((imgUrl, index) => (
                <div key={index} className="relative aspect-square rounded-md overflow-hidden border">
                  <Image
                    src={imgUrl}
                    alt={`Additional photo ${index + 1} of ${person.name || 'wanted person'}`}
                    layout="fill"
                    objectFit="cover"
                    data-ai-hint="person portrait"
